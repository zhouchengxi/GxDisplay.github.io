...
THREE.FBXLoader = (function () {
  ...
  FBXLoader.prototype = {
    ...
    load: function (url, onLoad, onProgress, onError) {
      ...
      // console.log('模型buffer', url,  buffer)
        try {
          scope.parse(buffer, path).then((res) => {
            onLoad(res)
          })
    ...
    parse: async function (FBXBuffer, path) {
      if (isFbxFormatBinary(FBXBuffer)) {
        if (window.Worker) {
          // 初始化webworker线程
          var workerPath = getHTMLPath() + 'workers/fbxworker.js'
          var myWorker = new Worker(workerPath)
  ...
  FBXTreeParser.prototype = {
    ...
    parseParameters: function (materialNode, textureMap, ID) {
      ...
      if (materialNode.Diffuse && typeof materialNode.Diffuse.value === 'string') {
        var diffuseList = materialNode.Diffuse.value.split(',')
        materialNode.Diffuse.value = diffuseList.map(function (item) {
          return Number(item)
        })
      }
      ...
      if (materialNode.Emissive && typeof materialNode.Emissive.value === 'string') {
        var emissiveList = materialNode.Emissive.value.split(',')
        materialNode.Emissive.value = emissiveList.map(function (item) {
          return Number(item)
        })
      }
      ...
      if (materialNode.Specular && typeof materialNode.Specular.value === 'string') {
        var specularList = materialNode.Specular.value.split(',')
        materialNode.Specular.value = specularList.map(function (item) {
          return Number(item)
        })
      }
    ...
    parseMorphTargets: function (relationships, deformerNodes) {
      ...
      for (var i = 0; i < relationships.children.length; i++) {
        ...
        var rawMorphTarget = {
          ...
          fullWeights: morphTargetNode.FullWeights && morphTargetNode.FullWeights.a
        ...
        if (rawMorphTarget.geoID && rawMorphTarget.geoID.ID) {
    ...
    parseScene: function (deformers, geometryMap, materialMap) {
      ...
      sceneGraph.traverse(function (node) {
        ...
        if (node.userData.transformData) {
          ...
          if (node.parent) {
            node.userData.transformData.parentMatrix = node.parent.matrix; // 20230105
            node.userData.transformData.parentMatrixWorld = node.parent.matrixWorld; // 20230105
          }
    ...
    createCurve: function (relationships, geometryMap) {
      ...
      // var material = new THREE.LineBasicMaterial({ color: 0xff9589, linewidth: 1 });
  ...
  function generateTransform3 (transformData) {
